...
export async function getDocuments<T>(collectionName: string, forceRefresh = false): Promise<T[]> {
  console.log(`Buscando documentos na coleção ${collectionName} - Força atualização: ${forceRefresh}`);
  try {
...
// Add a new document
export async function addDocument<T>(collectionName: string, data: any): Promise<T | null> {
  try {
    const docRef = await addDoc(collection(db, collectionName), {
      ...data,
      createdAt: serverTimestamp()
